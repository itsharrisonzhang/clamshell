...
// Forward declarations and defined structure hierarchy in sh61.cc
...
//        // `it.type()` returns the current tokenâ€™s type, which is one of
//        // the `TYPE_` constants.
//        // `it.str()` returns a C++ string holding the current tokenâ€™s
//        // character contents.
//    }
//    ```

struct shell_token_iterator {
    std::string str() const;    // current tokenâ€™s character contents
    inline int type() const;    // current tokenâ€™s type
